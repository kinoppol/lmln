...
Preferred: open `install.php` in a browser. [install.php](install.php) is a self-contained web installer — env checks, creates the database, drops any existing tables, runs `schema.sql`, writes `config/db.local.php`, runs `seed.php`, then replaces the seeded sample teacher with the admin account entered in the form (role `teacher`, so it reaches `teacher/dashboard.php`). It is **re-runnable**: a second install wipes and rebuilds, but only after an explicit confirmation checkbox. Because it can destroy the database, access is gated — localhost always passes, a never-installed system is open (so a fresh server can be set up), and once installed a remote caller must either hold a teacher session or supply the install key from `config/install_key.php` (generated and shown once at install time; `LQ_INSTALL_KEY` overrides). `LQ_DISABLE_INSTALLER=1` shuts it off entirely. It deliberately does *not* `require bootstrap.php`, because the `DB_*` constants must not be defined before the new connection config is written. All form validation runs before the first destructive statement, so a bad admin email can't leave a half-dropped database.
...
- [src/Auth.php](src/Auth.php) — session auth, `requireLogin()` / `requireTeacher()`. Roles are `student` | `general` | `teacher`; `student` additionally requires education level / major / school. A user with no `user_lesson_progress` rows sees every lesson as locked, so `Progress::ensureRows()` (one `INSERT IGNORE … SELECT` guarded by the `uq_user_lesson` unique key — never overwrites an existing status) backfills lesson 1 `unlocked` / rest `locked`. It runs on registration, when the installer and seed create teacher accounts, and on every `Auth::currentUser()` call, which self-heals accounts created outside registration and users who predate a newly added lesson.
...
[quiz.php](quiz.php) is one page handling all three kinds (`pretest` / `posttest` / `lesson`), driven by query string (`?kind=&lesson_id=&attempt=&q=`). It is a POST-redirect-GET loop: answering redirects to the next *unanswered* question, nothing is revealed along the way, and learners can jump back to any answered question (progress dots and a "ข้อก่อนหน้า" button) to change an answer — `submitAnswer()` upserts, so edits replace rather than duplicate. `q > total` is not the end; it renders a pre-submit review screen, and only an explicit `action=finish` POST grades the attempt and redirects to [quiz_result.php](quiz_result.php), which shows the whole answer key. Once `completed_at` is set every route back into `quiz.php` bounces to the result page, so answers can never be edited after the key has been seen.
